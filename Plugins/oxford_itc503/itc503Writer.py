...
class itc503Writer(object):
    """
    Class that holds the oxford itc503 controller logging and debugging. Creates a new file with the name: %Y-%m-%d_%H-%M-%S_%Keyword.log in the folder that was set. If the file already exists it tries to write the informations in the existing file.
    """
    def __init__(self, logger, keyword = None, **kwds):
        ...
        self.filename = os.path.join(self._logpath,"%s_%s_itc503.log" %(self.now.strftime('%Y-%m-%d_%H-%M-%S'),self.__keyword))
        ...
        self.write(("# Oxford ITC503 Controller logging file - generated %s. Logging mode is %s"%(self.now.strftime('%Y-%m-%d %H:%M:%S'),self.__keyword)))
