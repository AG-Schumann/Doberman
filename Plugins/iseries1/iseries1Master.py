...
import iseries1Serial
import iseries1Writer
import threading
import sys



class iseries1Master(object):
    """
    Main function to controll the iseries1 controller.
    The function can be called with several options in the command line mode.
    It will start automatically a serial connection to the iseries1 controller.
    It can run in interactive shells as well as a standalone python program (call via 'python iseries1Master.py -opts').
    """
    def __init__(self, opts, logger):
        ...
        self.controller = iseries1Serial.iseries1Serial(opts, logger)
        ...
        if hasattr(self.opts, 'log_path'):
            self.iseries1_writer = iseries1Writer.iseries1Writer(logger, self.opts.queue, log_path = self.opts.log_path)
        else:
            self.iseries1_writer = iseries1Writer.iseries1Writer(logger, self.opts.queue)

        if not self.opts.queue is None:
            self.logHead()

        self.writerThread = ReadoutThread(self.logger, self.opts, self.iseries1_writer, self.controller)

    def iseries1master(self):
        """
    ...
    def logHead(self):#TODO: add further parameters to loghead
        ...
        """
        self.iseries1_writer.writeToFile(str("# Reading from iseries1 Controller: The ID is %s . Its address is %s ."%(self.controller.getID(), self.controller.getAddress())))
        self.iseries1_writer.writeToFile(str("# Reading from iseries1 Controller: The high alarm values are: 1: %s ,2: %s ."%(self.controller.getAlarmHigh(1), self.controller.getAlarmHigh(2))))
        self.iseries1_writer.writeToFile(str("# Reading from iseries1 Controller: The low alarm values are: 1: %s ,2: %s ."%(self.controller.getAlarmLow(1), self.controller.getAlarmLow(2))))
       
        self.iseries1_writer.writeToFile(str("\n\n"))
        self.iseries1_writer.writeToFile("^ date (Y-m-d) ^ time (H:M:S) ^ Measured value ^")
        return
...
class ReadoutThread(threading.Thread):
    ...
    def ReadOutT(self):
        ...
        self.iseries1_writer.write(self.controller.getDisplayedValue(), now)



if __name__ == '__main__':
    parser = ArgumentParser(usage='%(prog)s [options] \n\n Program to readout the iseries1 controller')
    ...
    iseries1_master = iseries1Master(opts, logger)
    iseries1_master.iseries1master()
    sys.exit(0)
