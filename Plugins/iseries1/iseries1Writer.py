...
class iseries1Writer(object):
    """
    Class that holds the iseries1 controller logging and debugging. 
    ...
    def __init__(self, logger, queue=None, keyword = None, **kwds):
        ...
        self.filename = os.path.join(self._logpath,"%s_%s_iseries1.log" %(self.now.strftime('%Y-%m-%d_%H-%M-%S'),self.__keyword))
        ...
        self.writeToFile(("# iseries1 Controller logging file - generated %s. Logging mode is %s"%(self.now.strftime('%Y-%m-%d %H:%M:%S'),self.__keyword)))
    ...
    def pushToQueue(self, message, logtime):
        ...
        if str(data) == '-1':
            ...
            try:
                ...
                data = [0]
                status = [3]          
        self.queue.put(['iseries1',logtime,data,status])
        self.logger.debug("Put data to queue: ['iseries1', %s, %s, %s]"%(str(logtime),str(data),str(status)))
