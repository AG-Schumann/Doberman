...
import iseries3Serial
import iseries3Writer
import threading
import sys



class iseries3Master(object):
    """
    Main function to controll the iseries3 controller.
    The function can be called with several options in the command line mode.
    It will start automatically a serial connection to the iseries3 controller.
    It can run in interactive shells as well as a standalone python program (call via 'python iseries3Master.py -opts').
    """
    def __init__(self, opts, logger):
        ...
        self.controller = iseries3Serial.iseries3Serial(opts, logger)
        ...
        if hasattr(self.opts, 'log_path'):
            self.iseries3_writer = iseries3Writer.iseries3Writer(logger, self.opts.queue, log_path = self.opts.log_path)
        else:
            self.iseries3_writer = iseries3Writer.iseries3Writer(logger, self.opts.queue)

        if not self.opts.queue is None:
            self.logHead()

        self.writerThread = ReadoutThread(self.logger, self.opts, self.iseries3_writer, self.controller)

    def iseries3master(self):
        """
    ...
    def logHead(self):#TODO: add further parameters to loghead
        ...
        """
        self.iseries3_writer.writeToFile(str("# Reading from iseries3 Controller: The ID is %s . Its address is %s ."%(self.controller.getID(), self.controller.getAddress())))
        self.iseries3_writer.writeToFile(str("# Reading from iseries3 Controller: The high alarm values are: 1: %s ,2: %s ."%(self.controller.getAlarmHigh(1), self.controller.getAlarmHigh(2))))
        self.iseries3_writer.writeToFile(str("# Reading from iseries3 Controller: The low alarm values are: 1: %s ,2: %s ."%(self.controller.getAlarmLow(1), self.controller.getAlarmLow(2))))
       
        self.iseries3_writer.writeToFile(str("\n\n"))
        self.iseries3_writer.writeToFile("^ date (Y-m-d) ^ time (H:M:S) ^ Measured value ^")
        return
...
class ReadoutThread(threading.Thread):
    ...
    def ReadOutT(self):
        ...
        self.iseries3_writer.write(self.controller.getDisplayedValue(), now)



if __name__ == '__main__':
    parser = ArgumentParser(usage='%(prog)s [options] \n\n Program to readout the iseries3 controller')
    ...
    #print opts
    opts.addresses = [None ,opts.productID, opts.vendorID]
    #print opts

    iseries3_master = iseries3Master(opts, logger)
    iseries3_master.iseries3master()
    sys.exit(0)
