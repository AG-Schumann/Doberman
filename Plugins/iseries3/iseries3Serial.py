6#! /usr/bin/env python3.3

import serial
import subprocess
import iseries3Command
...
class iseries3Serial(iseries3Command.iseries3Command):
    """
    Class that holds the iseries3 controller serial connection. In total analogie to the cryoSerial which holts the cryoCon_22c
    ...
    def __init__(self, opts, logger, **kwds):
        ...
        super(iseries3Serial, self).__init__(**kwds)
...
## output of dmesg when connecting Newport (iseries3 T2)
# [ 2787.901250] usb 3-7: USB disconnect, device number 4
# [ 2787.901417] ch341-uart ttyUSB1: ch341-uart converter now disconnected from ttyUSB1
# [ 2787.901424] ch341 3-7:1.0: device disconnected
# [ 2805.160029] usb 3-7: new full-speed USB device number 12 using xhci_hcd
# [ 2805.300911] usb 3-7: New USB device found, idVendor=1a86, idProduct=7523
# [ 2805.300912] usb 3-7: New USB device strings: Mfr=0, Product=2, SerialNumber=0
# [ 2805.300913] usb 3-7: Product: USB2.0-Serial
# [ 2805.301549] ch341 3-7:1.0: ch341-uart converter detected
# [ 2805.302225] usb 3-7: ch341-uart converter now attached to ttyUSB1
    ...
    def get_ttyUSB(self,vendor_ID,product_ID, start_ttyUSB = 0):
        ...
        '''
        #print "iseries3  -> get_ttyUSB vendor_ID={}, product_ID={}, start_ttyUSB={}".format(vendor_ID, product_ID, start_ttyUSB)
        if start_ttyUSB >= 10:
    ...
    def checkController(self):
        ...
        !!! ATTENTION !!! THIS IS A TEMPERATURE CONTROLLER !!!
        """
        response = ''
        response = self.communicate("R05")
        #response = self.communicate("X01")
        #self.logger.debug(" communicate('X01').response = {}".format(response))
        #response = self._ID
        self.logger.debug(" communicate('R05').response = {}".format(response))
        if response == self._ID:
            ...
            try: #Adding to ttyusb list
                with open(os.path.join(self.opts.path, 'ttyUSB_assignement.txt'),"a+") as f:
                    f.write("    %d    |'iseries3 i3200'\n"%self.ttyUSB)
            except Exception as e:
                self.logger.warning("Can not add iseries3 i3200 to 'ttyUSB_assignement.txt'. Error %s"%e)
            finally:
                return 0
        elif response == -1:
            self.logger.warning("iseries3 controller is not answering correctly.")
            self.__connected = False 
            return -1

        elif  response != self._ID and len(response) == 4:
            self.logger.warning("ID not correct. Not the matching controller connected (Check ID on controller to make sure. Should be '%s' is '%s')"%(self._ID,response))
            self.__connected = False
            return -2
