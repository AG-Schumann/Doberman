...
class smartec_uti2Writer(object):
    """
    ...
    def __init__(self, logger, queue=None, keyword = None, **kwds):
        ...
        self.filename = os.path.join(self._logpath,"%s_%s_smartec_uti2.log" %(self.now.strftime('%Y-%m-%d_%H-%M-%S'),self.__keyword))
    ...
    def pushToQueue(self, message, logtime):
        ...
        if str(data) == '-1':
            ...
            try:
                ...
                data = [0]
                status = [3]          
        self.queue.put(['smartec_uti2',logtime,data,status])
        self.logger.debug("Put data to queue: ['smartec_uti2', %s, %s, %s]"%(str(logtime),str(data),str(status)))
...
if __name__ == '__main__':
    ...
    logger.setLevel(10)
    utiw = smartec_uti2Writer(logger)
