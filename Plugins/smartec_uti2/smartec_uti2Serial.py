...
import serial
import subprocess
import smartec_uti2Command
import smartec_uti2Config
...
class smartec_uti2Serial(smartec_uti2Command.smartec_uti2Command):
    """
    ...
    def __init__(self, opts, logger, **kwds):  
        ...
        self.opts = opts
        self.ttyUSB = -1
        self.mode = smartec_uti2Config.mode
        self.ref = smartec_uti2Config.ref
        self.bridgeref = smartec_uti2Config.bridgeref
        self.output = smartec_uti2Config.output
        self.wire = smartec_uti2Config.wire
        ...
        super(smartec_uti2Serial, self).__init__(**kwds)
    ...
    def get_ttyUSB(self,vendor_ID,product_ID, start_ttyUSB = 0):
        ...
        for ttyport in range(start_ttyUSB,10):
            ...
            if (tty_Vendor != [] and tty_Product != [] and tty_ID == "    ATTRS{serial}==\"DN01NAMY\"\n"):
                self.logger.info("Device with vendorID = '%s' and productID = '%s' and serialID = 'DN01NAMY' found at ttyUSB%d"%(vendor_ID, product_ID,ttyport))
                return ttyport
        self.logger.warning("Device with vendorID = '%s' and productID = '%s' and serialID = 'DN01NAMY' NOT found at any ttyUSB"%(vendor_ID, product_ID))       
        return -1
        ...
        try:
            ...
            if self.wire == 3:
                ...
                if valueb == valuea:
                    ...
                    ### Attention the formulas are wrong! Adjusted for 3 Capacitors mode:
                    #resp.append((valuea-valuec)/(valueb-valuec*1.0))
                    #print "  ********** *********  UTI2  ********** *************"
                    resp.append((valuec-valuea)/(valueb-valuea*1.0))
                    self.logger.debug("*** LLM_Xe (uti2) : A={}, B={}, C={} -> rel_cap={}   ***".format(valuea, valueb, valuec,resp[-1]) )
            if self.output >= 2:
                if valueb == valuea:
                    resp.append(0.0)
                else:
...
if __name__ == '__main__':
    ...
    utis = smartec_uti2Serial(logger)
