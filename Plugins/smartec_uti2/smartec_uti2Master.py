...
import smartec_uti2Serial
import smartec_uti2Writer
import threading
import sys



class smartec_uti2Master(object):
    """
    ...
    def __init__(self, opts, logger):
        ...
        self.controller = smartec_uti2Serial.smartec_uti2Serial(opts, logger)
        ...
        if hasattr(self.opts, 'log_path'):
            self.uti_writer = smartec_uti2Writer.smartec_uti2Writer(logger, self.opts.queue, log_path = self.opts.log_path)
        else:
            self.uti_writer = smartec_uti2Writer.smartec_uti2Writer(logger, self.opts.queue)
...
if __name__ == '__main__':
    ...
    uti_master = smartec_uti2Master(opts, logger)
