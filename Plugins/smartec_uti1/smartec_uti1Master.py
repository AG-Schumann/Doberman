...
import smartec_uti1Serial
import smartec_uti1Writer
import threading
import sys



class smartec_uti1Master(object):
    """
    ...
    def __init__(self, opts, logger):
        ...
        print "*************************** hier bin ich ******************************"
        self.logger = logger
        self.logger.info(" init of smartec_uti1.utiMaster...")
        self.logger.debug(" init of smartec_uti1.utiMaster...")
        self.opts = opts
        self.controller = None
        self.controller = smartec_uti1Serial.smartec_uti1Serial(opts, logger)
        ...
        if hasattr(self.opts, 'log_path'):
            self.uti_writer = smartec_uti1Writer.smartec_uti1Writer(logger, self.opts.queue, log_path = self.opts.log_path)
        else:
            self.uti_writer = smartec_uti1Writer.smartec_uti1Writer(logger, self.opts.queue)
...
class ReadoutThread(threading.Thread):
    ...
    def ReadOutT(self):
        ...
        try:
            __measure = self.controller.measure()
            self.logger.debug("  *** uti1.measure =  ", __measure)
            ## TODO: Adjust here to write 3 numbers!!!! ##
            self.uti_writer.write(__measure[0], now)
            #self.uti_writer.write(self.controller.measure(), now)
        except:
...
if __name__ == '__main__':
    ...
    uti_master = smartec_uti1Master(opts, logger)
