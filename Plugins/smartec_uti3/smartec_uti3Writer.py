...
class smartec_uti3Writer(object):
    """
    ...
    def __init__(self, logger, queue=None, keyword = None, **kwds):
        ...
        self.filename = os.path.join(self._logpath,"%s_%s_smartec_uti3.log" %(self.now.strftime('%Y-%m-%d_%H-%M-%S'),self.__keyword))
    ...
    def pushToQueue(self, message, logtime):
        ...
        if str(data) == '-1':
            ...
            try:
                ...
                data = [0]
                status = [3]          
        self.queue.put(['smartec_uti3',logtime,data,status])
        self.logger.debug("Put data to queue: ['smartec_uti3', %s, %s, %s]"%(str(logtime),str(data),str(status)))
...
if __name__ == '__main__':
    ...
    logger.setLevel(10)
    utiw = smartec_uti3Writer(logger)
