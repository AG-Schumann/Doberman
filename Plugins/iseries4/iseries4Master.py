...
import iseries4Serial
import iseries4Writer
import threading
import sys



class iseries4Master(object):
    """
    Main function to controll the iseries4 controller.
    The function can be called with several options in the command line mode.
    It will start automatically a serial connection to the iseries4 controller.
    It can run in interactive shells as well as a standalone python program (call via 'python iseries4Master.py -opts').
    """
    def __init__(self, opts, logger):
        ...
        self.controller = iseries4Serial.iseries4Serial(opts, logger)
        ...
        if hasattr(self.opts, 'log_path'):
            self.iseries4_writer = iseries4Writer.iseries4Writer(logger, self.opts.queue, log_path = self.opts.log_path)
        else:
            self.iseries4_writer = iseries4Writer.iseries4Writer(logger, self.opts.queue)

        if not self.opts.queue is None:
            self.logHead()

        self.writerThread = ReadoutThread(self.logger, self.opts, self.iseries4_writer, self.controller)

    def iseries4master(self):
        """
    ...
    def logHead(self):#TODO: add further parameters to loghead
        ...
        """
        self.iseries4_writer.writeToFile(str("# Reading from iseries4 Controller: The ID is %s . Its address is %s ."%(self.controller.getID(), self.controller.getAddress())))
        self.iseries4_writer.writeToFile(str("# Reading from iseries4 Controller: The high alarm values are: 1: %s ,2: %s ."%(self.controller.getAlarmHigh(1), self.controller.getAlarmHigh(2))))
        self.iseries4_writer.writeToFile(str("# Reading from iseries4 Controller: The low alarm values are: 1: %s ,2: %s ."%(self.controller.getAlarmLow(1), self.controller.getAlarmLow(2))))
       
        self.iseries4_writer.writeToFile(str("\n\n"))
        self.iseries4_writer.writeToFile("^ date (Y-m-d) ^ time (H:M:S) ^ Measured value ^")
        return
...
class ReadoutThread(threading.Thread):
    ...
    def ReadOutT(self):
        ...
        self.iseries4_writer.write(self.controller.getDisplayedValue(), now)



if __name__ == '__main__':
    parser = ArgumentParser(usage='%(prog)s [options] \n\n Program to readout the iseries4 controller')
    ...
    iseries4_master = iseries4Master(opts, logger)
    iseries4_master.iseries4master()
    sys.exit(0)
