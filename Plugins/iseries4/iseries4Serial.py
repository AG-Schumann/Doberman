...
class iseries4Serial(iseries4Command.iseries4Command):
    """
    Class that holds the iseries4 controller serial connection. In total analogie to the cryoSerial which holts the cryoCon_22c
    ...
    def __init__(self, opts, logger, **kwds):  
        ...
        super(iseries4Serial, self).__init__(**kwds)
        ...
        if response == self._ID:
            ...
            try: #Adding to ttyusb list
                with open(os.path.join(self.opts.path, 'ttyUSB_assignement.txt'),"a+") as f:
                    f.write("    %d    |'iseries4 i3200'\n"%self.ttyUSB)
            except Exception as e:
                self.logger.warning("Can not add iseries4 i3200 to 'ttyUSB_assignement.txt'. Error %s"%e)
            finally:
                return 0
        elif response == -1:
            self.logger.warning("iseries4 controller is not answering correctly.")
            self.__connected = False 
            return -1
