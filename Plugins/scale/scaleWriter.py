...
class scaleWriter(object):
    """
    Class that holds the scale logging and debugging. 
    ...
    def __init__(self, logger, queue=None, keyword = None, **kwds):
        ...
        self.filename = os.path.join(self._logpath,"%s_%s_scale.log" %(self.now.strftime('%Y-%m-%d_%H-%M-%S'),self.__keyword))
        ...
        self.writeToFile(("# scale logging file - generated %s. Logging mode is %s"%(self.now.strftime('%Y-%m-%d %H:%M:%S'),self.__keyword)))
    ...
    def pushToQueue(self, message, logtime):
        ...
        if str(data) == '-1':
            ...
            try:
                ...
                data = [0]
                status = [3]          
        self.queue.put(['scale',logtime,data,status])
        self.logger.debug("Put data to queue: ['scale', %s, %s, %s]"%(str(logtime),str(data),str(status)))
...
if __name__ == '__main__':
    ...
    logger.setLevel(10)
    scalew = scaleWriter(logger)
    scalew.write("test %s"%("test2"))
    scalew.close()
