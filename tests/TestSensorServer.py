...
class Thermosiphon(object):
    def __init__(self, dewar):
        ...
        self.T_liq = 78 # K
        self.Acs = np.pi * 0.075 ** 2 # 15cm diameter nitrogen reservoir
        self.Avent = np.pi * 0.01**2 # 2cm diameter vent
        self.reservoir_length = 1.5 # m, height of LN2 storage
        self.m_liq = 10 # kg
        self.m_gas = (p_amb+1) * self.Acs*(self.reservoir_length - self.liquid_level())/(R*self.T_gas) * nitrogen_molar_mass
    ...
    def cooling_power(self):
        ...
        p0 = 4*101325 # Pa, abs
        k = 1/(4*101325) # /Pa
        return max_cooling_power/(1+np.exp(-k*(self.p_active - p0))) * (self.m_liq > 0.001) * (self.p_active > p_amb*1.05)

    def liquid_level(self):
        """ in m """
        return self.m_liq/(self.Acs * self.rho_liq)
    ...
    def V_gas(self):
        """ in m^3 """
        return self.Acs * (self.reservoir_length - self.liquid_level())
    ...
    def vent_rate(self, dt=None):
        ...
        """
        dp = self.p_gas - p_amb
        dmdt = 0 if dp < 0 else self.Avent * np.sqrt(2 * dp * self.rho_gas)
    ...
    def step(self, dt, _print=False):
        ...
        if _print:
            print(f'TS {self.m_liq:.3f} kg | {dm_liquid/dt:.3g} kg/s | {self.m_gas:.3g} kg | {dm_gas/dt:.3g} kg/s | {(self.p_gas-p_amb)/100:.1f} mbar')
...
class IsoVac(object):
    def __init__(self, ic):
        ...
        self.pump_perf = np.array([(1, 100),(0.3, 110),(0.04,400),(0.01, 650),(0.005, 685),(0,685)], dtype=[('p', np.float32), ('rate', np.float32)])
        self.gas_conductivity = np.array([(0,0),(1e-7,1e-3),(1e-6,1e-2),(1e-5,0.1),(1e-4,1),(1e-3,10),(1e-2,85),(0.1,400),(1,700),(2000,700)],dtype=[('p', np.float32),('Q', np.float32)])
        self.ic = ic
    ...
    def step(self, dt, _print=False):
        ...
        self.m_gas += dm_gas
        if _print:
            print(f'IV pressure {self.pressure/100:.2g} mbar | rate {self.pump_rate():.2g} kg/s | {self.m_gas:.3g} kg')
